...
def create_text_index(es, index_name):
    mappings = {
        "properties": {"text": {"type": "text", "index": True}, "original_id": {"type": "integer"}}
    }
...
def read_dataset(dataset_name, split, messages_field, query_filter, query_field):
    dataset: Dataset = load_dataset(
        dataset_name, split=split, num_proc=open_instruct_utils.max_num_processes()
        ...
        for message in datum[messages_field]:
            if message[query_filter_key] == query_filter_value:
                data_to_index.append(
                    {"text": message[query_field], "metadata": datum, "original_id": i}
                )

    print(
        f"Read {dataset_name} for indexing. Has {len(dataset)} instances and {len(data_to_index)} messages."
    )
    return data_to_index


def stream_and_index_text(dataset_name, split, messages_field, query_filter, query_field,
                          es, index_name, text_batch_size):
    """Stream dataset from HF and index directly into ES without loading everything into memory."""
    dataset = load_dataset(dataset_name, split=split, streaming=True)
    query_filter_key, query_filter_value = query_filter.split(":")

    print(f"Streaming {messages_field} from {dataset_name} into {index_name}")
        ...
        for message in datum[messages_field]:
            if message[query_filter_key] == query_filter_value:
                bulk_data.append(
                    {
                        "_index": index_name,
                        "_source": {"text": message[query_field], "original_id": i},
                    }
                )
                if len(bulk_data) >= text_batch_size:
                    helpers.bulk(es, bulk_data)
                    total_indexed += len(bulk_data)
                    bulk_data = []
    ...
    print(f"Indexed {total_indexed} messages from {dataset_name} into {index_name}\n")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Batch size used if the `index_type` is `text`.",
    )
    parser.add_argument("--streaming", action="store_true",
                        help="Use streaming to avoid downloading full dataset to disk.")
    ...
    if args.dataset_mixer_config is not None:
        ...
        raise RuntimeError(
            "Specify a dataset or provide a train config file with dataset mixer info."
        )
