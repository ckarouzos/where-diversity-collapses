...
# Lazy-loaded globals (heavy dependencies loaded on first use)
_SPACY_MODEL = None
...
def exact_match(es, index_name, query_dataset, fields, search_size):
    ...
    for datum in tqdm(query_dataset):
        ...
        search_output = es.search(
            ...
            query={
                "bool": {
                    "filter": [{"match_phrase": {"text": query_str}} for query_str in query_strings]
                }
            },
        )
        ...
        if num_hits > 0:
            ...
            output_data.append(
                {"query": query_strings, "num_hits": num_hits, "train_docs": train_docs}
            )
        else:
...
def ngram_match(es, index_name, query_dataset, fields, ngram_size, search_size):
    ...
    for datum in tqdm(query_dataset):
        ...
        for query_string in query_strings:
            ...
            query_string_tokens = [d.text for d in _get_spacy_model()(query_string)]
...
def vector_match(
    es, index_name, query_dataset, fields, model, tokenizer, max_batch_tokens, search_size
):
    import torch
...
def main():
    ...
    if args.dataset_mixer_config is not None:
        ...
        print(
            f"Config has {len(dataset_names)} datasets. Looking for corresponding indexes: {index_names}"
        )
        ...
        raise RuntimeError(
            "Specify train_dataset_names or provide a train config file with dataset mixer info."
        )
    ...
    for index_name in index_names:
        ...
        for dataset, subset, split, fields, limit in eval_sets:
            ...
            if args.index_type == "text":
                if args.ngram_size is None:
                    ...
                    if args.match_threshold is not None:
                        match_scores = [
                            1 if score > args.match_threshold else 0 for score in match_scores
                        ]
                        contaminated_ids.update(
                            [
                                _id
                                for _id, score in train_indices_with_scores.items()
            ...
            output_filename = os.path.join(
                args.output_dir, f"{index_name}_{dataset.split('/')[-1]}.jsonl"
            )
    ...
    with open(output_file, "w") as outfile:
        ...
        for index_name, mean_match_scores in zip(index_names, all_index_match_scores):
            print(
                index_name
                + "\t"
                + "\t".join([f"{mean_match_scores[ev[0]]:.4f}" for ev in eval_sets]),
                file=outfile,
            )
    ...
    if args.decontaminate:
        ...
        for dataset_name, contaminated_ids in zip(dataset_names, all_index_contaminated_ids):
            ...
            train_dataset = load_dataset(
                dataset_name, split="train", num_proc=open_instruct_utils.max_num_processes()
            )
            ...
            output_path = os.path.join(
                args.output_dir, dataset_name.replace("/", "_") + "_decontaminated"
            )
